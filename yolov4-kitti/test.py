...
kitti_weights = 'checkpoints/kitti_best_v4.pth'
...
# Initiate model
model = Yolov4(yolov4conv137weight=kitti_weights, n_classes=num_classes, inference=True)
# model = Darknet(opt.model_config_path)
# model.load_weights(opt.weights_path)
pretrained_dict = torch.load(kitti_weights, map_location=torch.device('cpu'))
...
for c, ap in average_precisions.items():
    ...
    loss_data_file.write("%.5e "%ap)
mAP = np.mean(list(average_precisions.values()))
print(f"mAP: {mAP}")
loss_data_file.write("%.5e\n"% mAP)
